...
public class DocumentResumableUploadDemo {
	...
	private static final String[] USAGE_MESSAGE = { "Usage: java DocumentResumableUploadDemo.jar --username <user> --password <pass>", };
	 ...
	 * 
	 * @param docs
	 *            {@link DocumentList} for interface to DocList API service.
	 * @param out
	...
	public Collection<DocumentListEntry> uploadFiles(String url,
		...
		for (String fileName : files) {
			...
			ResumableGDataFileUploader uploader = new ResumableGDataFileUploader.Builder(
					docs.service, new URL(url), mediaFile, null /*
																 * empty
																 * meatadata
																 */)
	 ...
	 */
	private void executeUpdate(String[] args) throws IOException,
			ServiceException, InterruptedException {
		String docIdToUpdate = args[1];
		String filePath = args[2];

		// retrieve latest entry
		DocumentListEntry currentEntry = docs.service.getEntry(new URL(
				DEFAULT_DOCLIST_FEED_URL + "/" + docIdToUpdate),
				DocumentListEntry.class);

		MediaFileSource mediaFile = getMediaFileSource(filePath);
		ResumableGDataFileUploader uploader = new ResumableGDataFileUploader.Builder(
				docs.service, mediaFile, currentEntry)
		...
		DocumentListEntry updatedEntry = uploader
